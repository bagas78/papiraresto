...
$pdf->Cell(0, 5, 'LAPORAN PENGELUARAN BAHAN BAKU', 0, 1, 'C');
...
WHERE a.id_satuan = c.id_satuan AND a.id_kategori = b.id_kategori AND d.id_barang = a.id_barang AND d.jenis = 'Barang Keluar' AND SUBSTRING(d.tanggal, 1, 10) BETWEEN '$awal' AND '$akhir'";
$data = $this->db->query($sql)->result_array();

$pdf->Cell(30, 8, '', 0, 1);
$pdf->SetFont('Times', 'B', 9);
$pdf->Cell(30, 6, 'BARCODE', 1, 0, 'C');
$pdf->Cell(78, 6, 'NAMA ITEM', 1, 0, 'C');
...
foreach ($data as $d) {
    ...
    $pdf->Cell(78, 6, $d['nama_barang'], 1, 0);
    $pdf->Cell(17, 6, $d['satuan'], 1, 0);
    $pdf->Cell(17, 6, $d['jml'], 1, 0);
    $pdf->Cell(25, 6, 'Rp. ' . number_format($d['nilai'], '0', '.', '.'), 1, 0);
    $pdf->Cell(23, 6, $d['tanggal'], 1, 1);
}
$sql_nilai = "SELECT SUM(nilai) AS nilai FROM barang_operasional WHERE jenis = 'Barang Keluar' AND SUBSTRING(tanggal, 1, 10) BETWEEN '$awal' AND '$akhir'";
