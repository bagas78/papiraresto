...
$sql = "SELECT b.id_jual, b.invoice, d.nama_lengkap, c.nama_cs, SUM(a.diskon) AS diskon, SUM(a.subtotal) AS total, SUBSTRING(b.tgl, 1, 10) AS tgl, b.tgl AS waktu, SUM(a.qty_jual) AS qty FROM detil_penjualan a, penjualan b, customer c, user d  WHERE b.id_jual = a.id_jual AND c.id_cs = b.id_cs AND d.id_user = b.id_user AND b.is_active= 1 
...
foreach ($jual as $j) {
    ...
    $pdf->Cell(68, 6, 'NAMA MENU', 1, 0, 'C');
    $pdf->Cell(25, 6, 'HARGA', 1, 0, 'C');
    $pdf->Cell(17, 6, 'QTY', 1, 0, 'C');
    $pdf->Cell(35, 6, 'DISC (Rp.)', 1, 0, 'C');
    $pdf->Cell(36, 6, 'SUBTOTAL', 1, 1, 'C');
    $i = 1;
    foreach ($detil as $d) {
        if ($j['id_jual'] == $d['id_jual']) {
            ...
            $pdf->Cell(35, 6, $d['diskon'], 1, 0);
            $pdf->Cell(36, 6, $d['subtotal'], 1, 1);
        }
    }
    ...
    $pdf->Cell(28, 6, 'Rp. ' . $j['diskon'], 1, 1, 'L');
    $pdf->Cell(132, 6, '', 0, 0, 'R');
    $pdf->Cell(28, 6, 'Grand Total', 1, 0, 'L');
    $pdf->Cell(28, 6, 'Rp. ' . $j['total'], 1, 1, 'L');
}
