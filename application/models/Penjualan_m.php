...
class Penjualan_m extends CI_Model
{
    ...
    {
        $sql = "SELECT a.id_detil_jual, b.barcode, b.nama_barang, a.harga_item as harga_jual, a.qty_jual, a.diskon, a.subtotal FROM detil_penjualan a, menu b WHERE b.id_barang = a.id_barang AND a.id_jual IS NULL";
        return $this->db->query($sql)->result_array();
    }

    function cekHargaMinimum($id, $harga){
        
        $barang = $this->db->query("SELECT * FROM barang WHERE ID_BARANG = '$id'")->row_array();
    ...
    public function addItem($id, $qty, $subtotal, $harga, $diskon)
    {
        
        $hasil_diskon = $diskon / 100 * $subtotal;
        $hasil_subtotal = $subtotal - $hasil_diskon;
